...
- 核心概念放在第一行；如需解释，放第二行，**标题整体不超过两行**。
- 推荐结构：`核心概念` + `到底是什么？` / `怎么理解？` / `为什么重要？`。
- 封面不要先写长导语（如“让 AI 有据可查的关键技术”），这类说明放到副标题或正文框里。
...
- **AI知识标题检查**：核心概念必须突出，放第一行，标题最多两行；不要先写长导语或铺垫句。示例：`RAG` + `到底是什么？`。
