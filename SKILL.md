...
- RAG：`/Users/jiangziyi/Downloads/AI知识｜RAG到底是什么.html`
...
**AI知识模式：标题必须先让用户一眼看懂讲什么。**
- 核心概念必须突出显示，比如 `RAG` / `MCP` / `Function Calling` / `Embedding`，不要被铺垫句抢占视觉焦点。
- 顶部 `badge` 标签仍然保留在最上方；这里说的“第一行/第二行”指的是标签下面的正文主视觉标题区。
- 核心概念放在正文主视觉标题区第一行；如需解释，放第二行，**标题整体不超过两行**。
- 推荐结构：顶部标签 → `核心概念` → `到底是什么？` / `怎么理解？` / `为什么重要？`，参考已生成的 RAG 封面。
- 封面不要在核心概念前先写长导语（如“让 AI 有据可查的关键技术”），这类说明放到副标题或正文框里。

**AI日报/AI图文模式：标题优先吸引人。**
- 标题要抓点击：优先用数字冲击、反常识、冲突、悬念、人物/公司名。
- 可以不把技术名放第一行，但必须让用户一眼知道“为什么值得点开”。
- 避免平铺直叙的新闻标题，优先改成更有小红书传播感的表达。
